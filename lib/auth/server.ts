...
import { readCurrentSession, readPendingAccessSession } from "@/lib/auth/session";

export async function getOptionalServerSession(): Promise<AuthenticatedSession | null> {
  return readCurrentSession();
}

export async function getOptionalPendingAccessSession(): Promise<PendingAccessRequestContract | null> {
  return readPendingAccessSession();
}
